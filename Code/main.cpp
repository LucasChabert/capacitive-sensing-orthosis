...
const float d0     = 8.0f;  // distance de référence mesurée mécaniquement (mm)

// --- Variables de calibration (mesurées à l'origine) ---
float C10 = 0.0f;
float C20 = 0.0f;
float rapport_eps = 0.0f;  // epsilon1/epsilon2, constant
...
void setup() {
  ...
  rapport_eps = ((C10 / C20) -1 )* (d0 / (deltad));

  Serial.print(">C10 (pF):"); Serial.println(C10, 4);
  Serial.print(">C20 (pF):"); Serial.println(C20, 4);
  Serial.print(">rapport_eps:"); Serial.println(rapport_eps, 6);
...
void loop() {
  ...
  // --- Calcul de d avec epsilon corrigé ---
  float denominateur = rapport_eps * deltad *( 1/ ((C1 / C2) - 1.0f));
  Serial.print(">Distance d :"); Serial.println(denominateur, 6);
 Serial.print(">ration des c :"); Serial.println(C1/C2, 6);
 Serial.print(">ration des  eps:"); Serial.println(rapport_eps, 6);

  if (fabsf(denominateur) > 1e-6f) {
    float d = deltad / denominateur;
    Serial.print(">d (mm):"); Serial.println(d, 4);
  }
}